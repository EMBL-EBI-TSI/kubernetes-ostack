resource "null_resource" "ansible-provision" {
  ...
  provisioner "local-exec" {
    command =  "echo \"[kube-master]\n${openstack_compute_instance_v2.k8s-master.name} ansible_ssh_host=${openstack_compute_floatingip_v2.master-ip.address}\" > ../ansible/inventory.ini"
  }
  ...
  provisioner "local-exec" {
    command =  "echo \"\nbastion ansible_ssh_host=${openstack_compute_floatingip_v2.master-ip.address}\" >> ../ansible/inventory.ini"
  }
